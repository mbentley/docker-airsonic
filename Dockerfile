...
# get latest airsonic version from GitHub, check to make sure it hasn't passed the AIRSONIC_MAJOR_VER and install airsonic.war from https://github.com/airsonic-advanced/airsonic-advanced
RUN AIRSONIC_VER="$(if [ -z "${AIRSONIC_VER}" ]; then wget -q -O - "https://api.github.com/repos/airsonic-advanced/airsonic-advanced/releases?per_page=1" | jq -r '.[]|.tag_name'; else echo "${AIRSONIC_VER}"; fi)" &&\
